...
export function VisualizationDashboard({ dataset, onReset }: VisualizationDashboardProps) {
  ...
  const initializeChart = (index: number) => {
    ...
    const chartType = determineChartType(recommendation.type, recommendation.xAxis, recommendation.yAxis)
    
    setChartConfig({
      type: chartType,
      title: recommendation.title || `${recommendation.xAxis} vs ${recommendation.yAxis?.join(', ')}`,
  ...
  const determineChartType = (
    ...
    if (suggestedType && ['bar', 'line', 'scatter', 'pie', 'area', 'histogram'].includes(suggestedType)) {
    ...
    } else if (xColumnMeta?.type === 'categorical' || xColumnMeta?.type === 'string') {
      if (yAxis.length > 1) {
    ...
    } else if (xColumnMeta?.type === 'numeric' || xColumnMeta?.type === 'number') {
      if (yAxis.length > 1) {
  ...
  const createDefaultChart = () => {
    ...
    const numericColumn = columns.find(([_, meta]) => 
      meta.type === 'numeric' || meta.type === 'number'
    )
    const categoricalColumn = columns.find(([_, meta]) => 
      meta.type === 'categorical' || meta.type === 'string'
    )
    
    // Default to first two columns if specific types not found
    const xColumn = categoricalColumn?.[0] || columns[0]?.[0] || ''
    const yColumn = numericColumn?.[0] || (columns[1]?.[0] !== xColumn ? columns[1]?.[0] : columns[0]?.[0]) || ''
  ...
  const navigateRecommendation = (direction: 'next' | 'prev') => {
    ...
    if (direction === 'next') {
      ...
      newIndex = (activeRecommendation - 1 + dataset.recommendations.length) % dataset.recommendations.length
    }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="grid w-[400px] grid-cols-2 glass">
          <TabsTrigger value="visualize" className="data-[state=active]:neon-border">
            Visualize
          </TabsTrigger>
                        ...
                        <div>
                          ...
                          {dataset.recommendations && dataset.recommendations.length > 0 && (
                            <CardDescription className="opacity-70">
                              {dataset.recommendations[activeRecommendation]?.description || 'Visualize your data insights'}
                            </CardDescription>
                          )}
                        </div>
                        
                        {dataset.recommendations && dataset.recommendations.length > 1 && (
