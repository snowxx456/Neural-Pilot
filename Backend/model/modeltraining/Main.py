...
target = TargetColumnRecommender(file_path="automl\c.csv")
...
if target.load_data():
    ...
    if target._validate_data():
        ...
        if dataloader.load_data():
            ...
            visualization = VisualizationHandler(data=df, best_model=model_training.best_model, best_model_name=model_training.best_model_name, results=model_training.results, 
                                                 feature_names=dataloader.feature_names, label_encoder=dataloader.label_encoder, 
                                                 probabilities=model_training.results[model_training.best_model_name].get('probabilities'), y_test=model_training.y_test, problem_type=dataloader.problem_type)
            ...
            print(visualization.compare_model_plot())
